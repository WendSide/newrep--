...
// === PHP ДЛЯ ОТПРАВКИ НОВЫХ КАРТ В БД ===
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Обработка фото
        // Изначально переменная для пути к фото равна null
        $photo_path = null;
        // Проверяем, был ли загружен файл с ключом 'photo' и не возникли ли ошибки при загрузке
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, $allowed)) {
                // Генерирует уникальное имя файла с текущим временем и случайным числом
